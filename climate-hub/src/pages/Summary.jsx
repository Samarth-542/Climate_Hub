...
export default function Summary() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-8">
      <h1 className="text-2xl font-bold text-slate-100 mb-6">Climate Intelligence</h1>
      ...
      <div className="bg-slate-900 rounded-2xl shadow-xl border border-slate-800 overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-8 text-white relative overflow-hidden">
            ...
            <p className="opacity-90 relative z-10">Analyzing {incidentsCount} incidents from the last 24 hours.</p>
        </div>
        ...
        <div className="p-8 min-h-[300px]">
            {loading ? (
                <div className="flex flex-col items-center justify-center h-full py-12 text-slate-400">
                    <RefreshCw className="animate-spin mb-4 text-emerald-500" size={40} />
                                    ...
                                    <div key={i} className={clsx("mb-2", line.startsWith('-') && "ml-4")}>
                                        {parts.map((part, j) => {
                                            if (part.startsWith('**') && part.endsWith('**')) {
                                                return <strong key={j} className="text-emerald-400">{part.slice(2, -2)}</strong>;
                                            } else if (part.startsWith('###')) {
                                                return <h3 key={j} className="text-xl font-bold text-slate-100 mt-4 mb-2">{part.replace('###', '').trim()}</h3>;
                                            }
                                            return part;
                                        })}
                                    </div>
        ...
        <div className="p-6 border-t border-slate-800 bg-slate-900 flex justify-end">
            <button 
                ...
                disabled={loading}
                className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-lg font-medium transition disabled:opacity-50 shadow-lg shadow-emerald-900/20"
            >
